...
export default function ServiceEngineerITServiceTicket() {
  ...
  const [inputServiceRenderedDialogOpen, setInputServiceRenderedDialogOpen] =
    useState(false);
  const [ITSMFormDialogOpen, setITSMFormDialogOpen] = useState(false);
  const [
    updateStatusAssignedTicketDialogOpen,
    setUpdateStatusAssignedTicketDialogOpen,
  ] = useState(false);
  ...
  const inputFindingsDialogMutation = useMutation({
    ...
    mutationFn: async (data: string) => {
      ...
      return await api.put(`/service-tickets/${parsedData.id}`, {
        defectsFound: parsedData.findings,
      });
    },
    onSuccess: async () => {
  ...
  const updateStatusAssignedTicketDialogMutation = useMutation({
    ...
    mutationFn: async (data: string) => {
      const parsedData = JSON.parse(data);
      return await api.patch(`/service-tickets/${parsedData.id}/status`, {
        serviceStatus: parsedData.serviceStatus,
        notes: `Status updated to ${parsedData.serviceStatus}`,
      });
    },
    onSuccess: async () => {
      queryClient.invalidateQueries({ queryKey: queryKey });
    ...
    <section className="grid custom-md:grid-cols-1 gap-4">
      <div className="text-xl font-semibold m-0">
        <div className="flex justify-between items-center text-xl font-semibold m-0 mb-6">
          ...
          <Button
            variant="outline"
            onClick={() => navigate("/service-engineer")}
          >
            <Undo2 />
            Back
          </Button>
        ...
        {!serviceTicket?.rating && (
          <>
            ...
            <Button
              variant="outline"
              type="submit"
              {...((serviceTicket?.serviceStatus === "on hold" ||
                serviceTicket?.serviceStatus === "closed") && {
                disabled: true,
              })}
              onClick={() => {
          ...
          <CardContent>
            <div className="grid gap-3 items-stretch h-full ">
              <div className="grid gap-2">
                ...
                <div className="border p-3 h-auto rounded-md text-sm bg-gray-100 max-h-24 overflow-auto overflow-x-hidden">
              ...
              <div className="grid gap-2">
                <span className="text-sm text-gray-500 font-semibold">
                  Findings&nbsp;
                  {serviceTicket?.serviceStatus !== "closed" && (
                    <span className="text-xs">(click the box below)</span>
                  )}
                </span>
                <div
                  className={`border p-3 h-16 rounded-md text-sm ${
                    serviceTicket?.serviceStatus !== "closed"
                  ...
                  onClick={
                    serviceTicket?.serviceStatus !== "closed"
                      ? () => setInputFindingsDialogOpen(true)
                      : undefined
                  }
                >
              ...
              <div className="grid gap-2">
                ...
                <div
                  ...
                  onClick={
                    serviceTicket?.serviceStatus !== "closed"
                      ? () => setInputServiceRenderedDialogOpen(true)
                      : undefined
                  }
                >
